...
class FrostedGlassView @JvmOverloads constructor(
    ...
    private fun setupRefreshAnimation() {
        ...
        refreshAnimator?.addListener(object : android.animation.Animator.AnimatorListener {
            ...
            override fun onAnimationEnd(animation: android.animation.Animator) {
                // 5秒后安排下一次刷新
                postDelayed({ setupRefreshAnimation() }, 5000)
            }
    ...
    private fun drawRefreshEffect(canvas: Canvas) {
        ...
        // 从左上角之前开始，到右下角之后结束
        val startOffset = -diagonalLength
        val endOffset = diagonalLength
        val currentOffset = startOffset + refreshProgress * (endOffset - startOffset)
        
        // 计算刷新效果的宽度（1/4对角线长度）
        val refreshWidth = diagonalLength / 4
        
        // 创建刷新效果的线性渐变
        // 使用更窄的渐变以创建1/4宽度的效果
        val gradient = LinearGradient(
            currentOffset - refreshWidth, currentOffset - refreshWidth,
            currentOffset + refreshWidth, currentOffset + refreshWidth,
            intArrayOf(
                Color.TRANSPARENT,
                Color.argb(128, 255, 255, 255), // 50%透明白色
                Color.TRANSPARENT
            ),
            floatArrayOf(
                0f,
        ...
        // 绘制刷新效果
        canvas.drawPath(refreshPath, refreshPaint)
    }
